...
import type { EnrichedModel, CreateActionData, UpdateActionData, StateUpdateMessage, ModelConfig, ProviderType, SetupDomain, SetupModule } from '../types.js';
...
import { SetupEngine } from '../runtime/index.js';
import template from '../templates/index.js';

export class Controller extends AgwViewProviderBase {
  public static readonly viewType = 'keyView';

  private readonly router = new SetupRouter();
  private readonly engine = new SetupEngine();
  ...
  private async renderHtml(webviewView: WebviewView): Promise<void> {
    ...
    const scriptUri = webviewView.webview.asWebviewUri(
      Uri.joinPath(this.context.extensionUri, 'dist', 'extension', 'modules', 'setup', 'web', 'setup-view.js')
    );
  ...
  private async syncState(): Promise<void> {
    if (!this.webviewView) {
      console.log('[SetupController] syncState - No webviewView');
      return;
    }
  ...
  protected override async onReady(): Promise<void> {
    console.log('[SetupController] onReady - Webview is ready');
...
/**
 * Creates and registers the setup domain.
 * This function resides in the background scope because it depends on VS Code API.
 */
export function createSetupDomain(context: ExtensionContext): SetupDomain {
  ...
  const view = new Controller(context, apiKeyBroadcaster, settingsStorage);
