...
2. Crear el módulo (module-agent)
   - El **module-agent** usa su skill `extensio_create_module`:
     - Leer `.agent/skills/extensio-create-module/SKILL.md`
     - Ejecutar MCP tool `mcp_extensio-cli_extensio_create` con:
       - `type: "module"`
       - `name`: nombre del módulo
       - `withShards`: según necesidades de UI
       - `withPages`: según necesidades de páginas
       - `includeDemo`: true (default)
...
7. Solicitar aprobacion del desarrollador (OBLIGATORIA, por consola)
   - El desarrollador **DEBE** aprobar el informe:
     - **SI** → aprobado
     - **NO** → rechazado
   - Registrar la decision en `module/create.md`:
     ```yaml
     approval:
       developer:
         decision: SI | NO
...
8. PASS
   ...
   - Actualizar `.agent/artifacts/<taskId>-<taskTitle>/task.md`:
     - Agregar una entrada en `task.lifecycle.subflows.modules.create[]`:
       - `name: <module-name>`
       - `completed: true`
...
9. Declarar creación de módulo como **NO completada**
...
3. El `module/create.md` inicia con el prefijo del `module-agent`.
4. El módulo cumple `constitution.modules`.
5. Si hay Pages, cumplen `constitution.pages` (especialmente §11-12).
6. Si hay Shards, cumplen `constitution.shards`.
7. Auditoría del architect-agent aprobada.
8. Existe aprobacion explicita del desarrollador (por consola) registrada en `module/create.md`:
   - `approval.developer.decision == SI`
9. `task.md` refleja subflow y timestamp:
   - Existe una entrada en `task.lifecycle.subflows.modules.create[]` con:
     - `name: <module-name>`
     - `completed == true`
