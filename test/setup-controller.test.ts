...
import { Controller } from '../src/extension/modules/setup/background/index.js';

const mockApiKeyBroadcaster = { notify: vi.fn(), dispose: vi.fn(), onDidChange: vi.fn(() => ({ dispose: vi.fn() })) };
const mockSettingsStorage = {
  getConfig: vi.fn(() => ({ models: [], activeModelId: undefined, artifactsPath: undefined })),
  getModels: vi.fn(() => []),
  setModels: vi.fn(),
...
describe('Setup Controller', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    mockSettingsStorage.getConfig.mockReturnValue({ models: [], activeModelId: undefined, artifactsPath: undefined });
    controller = new Controller(mockContext, mockApiKeyBroadcaster as any, mockSettingsStorage as any);
  });
  ...
  it('should refresh state when model is edited', async () => {
    ...
    mockSettingsStorage.getConfig.mockReturnValue({ models, activeModelId: 'm1' });
    (controller as any).webviewView = mockWebviewView;
  ...
  it('should handle model creation and redirect', async () => {
    (controller as any).webviewView = mockWebviewView;
    const data = { name: 'New', provider: 'openai', modelId: 'gpt-4' };
  ...
  it('should handle model deletion', async () => {
    ...
    mockSettingsStorage.setModels.mockImplementation(async (newModels: any) => {
      mockSettingsStorage.getConfig.mockReturnValue({ models: newModels, activeModelId: undefined, artifactsPath: undefined });
    });
  ...
  it('should handle model update', async () => {
    ...
    mockSettingsStorage.getConfig.mockReturnValue({ models, activeModelId: 'm1', artifactsPath: undefined });
    (controller as any).webviewView = mockWebviewView;

    (controller as any).router.setEditingModel('m1', 'openai');

    const updateData = { name: 'New Name', provider: 'gemini', modelId: 'pro' };
